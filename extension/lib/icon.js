...
const STATES = {
  off: {
    ...
    badge: '',
    badgeColor: '#000000',
    tooltipFn: () => 'PAPA PROXY — disabled',
  },
  routed: {
    iconBase: 'icons/routed',
    badgeColor: '#10b981',
    tooltipFn: ({ host, country, latencyMs }) =>
      `PAPA PROXY — ${host} routed via proxy${country ? ' (' + country + ')' : ''}${latencyMs ? ' · ' + latencyMs + ' ms' : ''}`,
  },
  direct: {
    iconBase: 'icons/direct',
    badge: '',
    badgeColor: '#000000',
    tooltipFn: ({ host }) => `PAPA PROXY — ${host} is direct (not in routed list)`,
  },
  error: {
    iconBase: 'icons/error',
    badge: '!',
    badgeColor: '#ef4444',
    tooltipFn: ({ reason }) => `PAPA PROXY — proxy error: ${reason || 'unreachable'}`,
  },
};
